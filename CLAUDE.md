...
- **Builds for non-production branches are OFF** in the Cloudflare dashboard (mnemradev Worker → Settings → Build → Branch control). A branch build runs the branch's own code — npm install scripts, the build script — with the Workers Builds token, which by default can deploy, so a pushed branch (Dependabot's included) could reach production. ⚠ This lives in the dashboard: nothing in the repo records or checks it. If a PR head ever shows a `Workers Builds: mnemradev` check again, the toggle was turned back on.
- **`wrangler.jsonc` sets `"preview_urls": false`**, and `just verify` fails if that line is removed or changed. With previews on, any build that uploads a version serves it at a public, unauthenticated `workers.dev` URL whose alias is guessable from the branch name (measured 2026-09-15). Wrangler applies the flag on `wrangler deploy` from `main`, not on a branch upload, so a change to it is verified after merge. A dashboard toggle for previews is overwritten by the next deploy if the file disagrees.
