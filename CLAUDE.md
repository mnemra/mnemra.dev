...
**Pushing a branch must not publish a preview of it.** `wrangler.jsonc` sets `"preview_urls": false`, and `just verify` fails if that line is removed or changed. The flag takes effect when `wrangler deploy` runs from `main`: a branch build reads it but does not apply it (wrangler 4.111.0 source), so a change to it is verified after merge, not on the branch. Without it, every branch build uploads a version reachable at a public, unauthenticated `workers.dev` preview URL whose alias is guessable from the branch name, so pushing an unreviewed draft would put it online before anyone merged it (measured 2026-09-15, #3606). Keep that line; the Cloudflare dashboard toggle is overwritten by the next deploy if the file disagrees.
