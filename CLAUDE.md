...
- **Present tense covers Mnemra through V0.1, as the locked mnemra-core product brief defines it.** The brief lives in that repo at `docs/_published/agent/intent/mnemra-core.md`: V0 (`1.0.0`) is the internal cutover, and V0.1 (`1.1.0`) is where the core product promise activates. The site describes that product before it ships, so the reference for a claim is the brief's V0 and V0.1 register entries, not today's code; where the code differs from the brief, that is a defect to file against mnemra-core rather than a reason to change the copy. A claim needs both conditions to take present tense: it is in the brief, and it is in V0 or V0.1. Anything else, including a later entry in the same register, is marked as intent in the sentence itself — "we're building toward", "target architecture", "planned for 1.3" — never present-tense-by-implication. Declarative phrasing and the bans on hedging govern *how* a claim is worded; neither one licenses present tense past that line.
- **Until it ships, the page says so in the same section as the claim.** A present-tense description of an unreleased product with no plain statement nearby of what is and isn't out yet is the omission described in the next bullet, and a status line in the footer does not cover it.
...
- **Declarative** — "Mnemra is X," not "Mnemra can be seen as X"; avoid hedging verbs (seems, might, could be) when you can just say it, and when X falls inside the present-tense line the Truth block draws. Where it doesn't, the declarative sentence is about the intent ("we're building X"), not about X.
