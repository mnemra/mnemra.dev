...
**There is no GitHub Actions CI in this repo, and a PR gets no Cloudflare build.** No status check is required to merge. `just check` on your branch is the only gate that runs `astro check` and the verify harness before a change goes live. Nothing else will catch a break for you.

**Pushing a branch must not publish or deploy anything. Two layers hold that, and each is independently load-bearing (#3606):**
- **Builds for non-production branches are OFF** in the Cloudflare dashboard (mnemradev Worker → Settings → Build → Branch control). A branch build runs the branch's own code — npm install scripts, the build script — with the Workers Builds token, which by default can deploy, so a pushed branch (Dependabot's included) could reach production. ⚠ This lives in the dashboard: nothing in the repo records or checks it. **Parked, checked at each landing:** before merging a PR, `gh api repos/mnemra/mnemra.dev/commits/<PR head sha>/check-runs --jq .total_count` must print `0`; anything else means the toggle was turned back on.
- **`wrangler.jsonc` sets `"preview_urls": false`**, and `just verify` fails unless both that file and the generated config `wrangler deploy` actually reads (via `.wrangler/deploy/config.json`) carry it. With previews on, any build that uploads a version serves it at a public, unauthenticated `workers.dev` URL whose alias is guessable from the branch name (measured 2026-09-15). Wrangler applies the flag on `wrangler deploy` from `main`, not on a branch upload, so a change to it is verified after merge. A dashboard toggle for previews is overwritten by the next deploy if the file disagrees.
...
- **Present tense is reserved for what has shipped and is runnable.** Mnemra has no release, so today everything about the product is forthcoming and the sentence says so — "will", "we're building", "planned". Never present-tense-by-implication. Declarative phrasing and the bans on hedging govern *how* a claim is worded; neither one licenses present tense for something unshipped.
- **What is forthcoming comes from mnemra-core's documentation, not its code.** The product brief (`docs/_published/agent/intent/mnemra-core.md` in that repo), the locked frames and specs it points to, and the plans and tasks behind them are the source for what Mnemra will be. Describe intended features in the order they will ship. A capability no document places anywhere is not described at all. Where the code differs from the documents, that is a defect to file against mnemra-core rather than a reason to change the copy.
- **Each surface states that it hasn't shipped, on its own.** The meta description, the hero, a feature list: each is read without the others — a search result shows the meta description alone — so each carries the status itself rather than leaning on another. A line in the footer does not cover the page.
- **The standing publishing decisions are in `~/work/brain/projects/mnemra/2026-07-15-blog-outline-gate-decisions.md`.** Positioning frame, and the disclosure calls on what may be revealed publicly. Read it before drafting or reviewing any public Mnemra prose; it settles questions this file does not.
...
- **Declarative** — "Mnemra is X," not "Mnemra can be seen as X"; avoid hedging verbs (seems, might, could be) when you can just say it, and when X has shipped. Where it hasn't, the declarative sentence is about the intent ("we're building X"), not about X.
